...
f = lambda x: torch.sin(3*x) + torch.cos(x) + 0.1 * torch.randn(x.size())
f_actual = lambda x: torch.sin(3*x) + torch.cos(x)
...
actual_y = torch.tensor([f_actual(p) for p in x_plot])
